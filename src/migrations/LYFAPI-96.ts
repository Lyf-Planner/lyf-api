...
const migrateItemInCollection = async (
  ...
  var last_updated = new Date(item.last_updated).toISOString();
  var created = new Date(item.created).toISOString();
  console.log("\tSetting created to", created, "last_updated to", last_updated);
  await lyf_db
